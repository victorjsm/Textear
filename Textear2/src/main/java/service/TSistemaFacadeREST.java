 ...
 */
@Stateless
@Path("classes.tsistema")
public class TSistemaFacadeREST extends AbstractFacade<TSistema> {

    @PersistenceContext(unitName = "com.mycompany_Textear2_war_1.0-SNAPSHOTPU")
    private EntityManager em;

    private TSistemaPK getPrimaryKey(PathSegment pathSegment) {
        /*
         ...
         */
        classes.TSistemaPK key = new classes.TSistemaPK();
    ...
    public TSistemaFacadeREST() {
        super(TSistema.class);
    }
    
    // GET Tarea Sistema
    // Procedimiento por el cual el REST recibe el rif de la empresa a travez de
    // una llamada GET y con el devuelve todas las tareas de sistema asociadas 
    // a ese rif en un arreglo JSON.
    
    @GET
    @Path("/TsistemaGET/{rif}")
    @Produces({MediaType.APPLICATION_JSON})
    public String find_sistemas(@PathParam("rif") String rif) {
        ...
        try {
            ...
            List lista = em.createNativeQuery("SELECT DISTINCT Nombre FROM t_sistema WHERE "
            ...
            for (int i = 0; i < lista.size(); i++) {
                ...
                List<TSistema> temp = super.findWithQuery(
                ...
                for (int k = 0; k < temp.size(); k++) {
                    ...
                    if (k == 0) {
                        ...
                        tarea.put("nombre", temp.get(k).getTSistemaPK().getNombre());
                    ...
                    abonado.put("telefono", temp.get(k).getTSistemaPK().getTelefono());
    ...
    public String createtsistema(String temp) throws ParseException, JSONException {
        ...
        try {
            ...
            for (int i = 0; i < arreglo.length(); ++i) {
                ...
                TSistema tsis = new TSistema();
                TSistemaPK tsispk = new TSistemaPK();
                ...
                //------ DATOS DE LA TAREA
                tsis.setBandeja(men.getString("bandeja"));
                tsis.setCodigoCanal(can);

                tsis.setEstado("PENDIENTE");

                tsispk.setTelefono(json.getString("telefono"));
                tsispk.setNombre(men.getString("nombre"));
                tsispk.setRifEmpresa(json.getString("rif"));
                tsis.setFechaCreacion(today);
                tsis.setFechaExpiracion(expiro);
                tsis.setFechaEnvio(envio);
                tsis.setTSistemaPK(tsispk);
                
                
                tsis.setAbonado(abo);
                               
                if (super.find(tsispk) == null) {
                    super.create(tsis);
                } else {
...
//            java.util.logging.Logger.getLogger(WebController.class.getName()).log(Level.SEVERE, null, ex);
    ...
    @POST
    @Path("/tsistemaEditPOST")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String edittsistema(String temp) throws ParseException, JSONException {
        ...
        try {
            ...
            for (int i = 0; i < arreglo.length(); ++i) {
                ...
                tsispk.setTelefono(json.getString("telefono"));
                tsispk.setNombre(men.getString("nombre"));
                tsispk.setRifEmpresa(json.getString("rifEmpresa"));
...
//            java.util.logging.Logger.getLogger(WebController.class.getName()).log(Level.SEVERE, null, ex);
    ...
    @POST
    @Path("/tsistemaElimPOST")
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String eliminartsistema(String temp) throws JSONException, ParseException {
            ...
            for (int i = 0; i < tareas.length(); ++i) {
                ...
                for (int j = 0; j < arreglo.length(); ++j) {
                    ...
                    if (super.find(tsispk) == null) {
                        ...
                        super.remove(super.find(tsispk));
                    }
                }
            }
